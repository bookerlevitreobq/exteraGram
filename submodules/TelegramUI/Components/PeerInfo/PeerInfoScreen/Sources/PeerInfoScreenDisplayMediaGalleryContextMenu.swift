        ...
        if isBotPreviewOrStories {
            ...
            |> deliverOnMainQueue).startStandalone(next: { [weak self] messageCounts in
                ...
                if strongSelf.chatLocation.threadId == nil {
                    let isCalendarAvailable = pane.sorting == .date
                    items.append(.action(ContextMenuActionItem(text: strings.SharedMedia_ShowCalendar, icon: { theme in
                        return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Calendar"), color: isCalendarAvailable ? theme.contextMenu.primaryColor : theme.contextMenu.primaryColor.withMultipliedAlpha(0.4))
                    }, action: isCalendarAvailable ? { _, a in
                        if ignoreNextActions {
                            return
                        }
                        ignoreNextActions = true
