...
private func peerInfoAvailableMediaPanes(context: AccountContext, peerId: PeerId, chatLocation: ChatLocation, isMyProfile: Bool, chatLocationContextHolder: Atomic<ChatLocationContextHolder?>, sharedMediaFromForumTopic: (EnginePeer.Id, Int64)?) -> Signal<[PeerInfoPaneKey]?, NoError> {
    ...
    let functionStartTime = CFAbsoluteTimeGetCurrent()
    Logger.shared.log("PeerInfoTabs", "peerInfoAvailableMediaPanes called: peerId=\(peerId) isMyProfile=\(isMyProfile) sharedMediaFromForumTopic=\(sharedMediaFromForumTopic != nil ? "yes" : "no") chatLocation=\(chatLocation)")

    var tags: [(MessageTags, PeerInfoPaneKey, String)] = []

    if !isMyProfile {
        tags = [
            (.photoOrVideo, .media, "photoOrVideo"),
            (.file, .files, "file"),
            (.music, .music, "music"),
            (.voiceOrInstantVideo, .voice, "voice"),
            (.webPage, .links, "links"),
            (.gif, .gifs, "gifs"),
            (.polls, .polls, "polls")
        ]
    ...
    return combineLatest(queue: .mainQueue(), tags.map { tagAndKey -> Signal<(PeerInfoPaneKey, PaneState), NoError> in
        ...
        |> map { (view, _, _) -> (PeerInfoPaneKey, PaneState) in
            ...
            if view.entries.isEmpty {
                if view.isLoading {
                    Logger.shared.log("PeerInfoTabs", "  TAG \(tagName): LOADING (entries=0 isLoading=1 elapsed=\(String(format: "%.2f", elapsed))s)")
                    return (key, .loading)
                } else {
                    Logger.shared.log("PeerInfoTabs", "  TAG \(tagName): EMPTY (entries=0 isLoading=0 elapsed=\(String(format: "%.2f", elapsed))s)")
                    return (key, .empty)
                }
            } else {
    ...
    |> map { keysAndStates -> [PeerInfoPaneKey]? in
        ...
        var loadingTags: [String] = []
        var presentTags: [String] = []
        var emptyTags: [String] = []
        for ((_, _, tagName), (key, state)) in zip(tags, keysAndStates) {
            switch state {
            case .present:
                result.append(key)
                presentTags.append(tagName)
            case .empty:
                emptyTags.append(tagName)
            case .loading:
                hasNonLoaded = true
                loadingTags.append(tagName)
            }
        }
        let totalElapsed = CFAbsoluteTimeGetCurrent() - functionStartTime
        if !hasNonLoaded || loadedOnceValue {
            if !loadedOnceValue {
                let _ = loadedOnce.swap(true)
                Logger.shared.log("PeerInfoTabs", "ALL RESOLVED FIRST TIME. present=[\(presentTags.joined(separator: ","))] empty=[\(emptyTags.joined(separator: ","))] elapsed=\(String(format: "%.2f", totalElapsed))s → tabs=[\(result.map(\.rawValue).map(String.init).joined(separator: ","))]")
            } else {
                Logger.shared.log("PeerInfoTabs", "COMBINE (loadedOnce=1). present=[\(presentTags.joined(separator: ","))] empty=[\(emptyTags.joined(separator: ","))] loading=[\(loadingTags.joined(separator: ","))] elapsed=\(String(format: "%.2f", totalElapsed))s → tabs=[\(result.map(\.rawValue).map(String.init).joined(separator: ","))]")
            }
            return result
        } else {
            Logger.shared.log("PeerInfoTabs", "RETURN NIL — stuck loading tags=[\(loadingTags.joined(separator: ","))] present=[\(presentTags.joined(separator: ","))] empty=[\(emptyTags.joined(separator: ","))] loadedOnce=\(loadedOnceValue) elapsed=\(String(format: "%.2f", totalElapsed))s")
            return nil
        }
    }
    |> distinctUntilChanged
